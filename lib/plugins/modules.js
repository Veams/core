	   ...
	   */
			...
			value: function initModules(domName, Module, render, options, cb) {
				...
				forEachHelper(this.modulesInContext, function (i, el) {
					_this2.initModule(el, domName, Module, render, options, cb);
				});
			}
		}, {
			key: 'initModule',
					...
					for (var i = 0; i < mutations.length; ++i) {
						...
						for (var j = 0; j < mutations[i].addedNodes.length; ++j) {
							...
							if (addedNode instanceof HTMLElement) {
								if (addedNode.getAttribute(_this3.options.attrPrefix + '-module')) {
									var domName = addedNode.getAttribute(_this3.options.attrPrefix + '-module');
									...
									try {
										for (var _iterator = _this3.modulesList[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
											var module = _step.value;
	
											if (module.domName === domName) {
	
												_this3.initModule(addedNode, module.domName, module.module, module.render, module.options, module.cb);
								...
								if (_this3.getModulesInContext(addedNode)) {
									_this3.modulesInContext = _this3.getModulesInContext(addedNode);
